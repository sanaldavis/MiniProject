...
def sample1():
	...
	n=e5.get()
	n=int(n)
	db1 = MySQLdb.connect(host="localhost",user="root",passwd="shaheer",db="result")
	cursor = db1.cursor()
...
def display():
	...
	semstr=sem+branch+batch		
	class Application(Frame):
		def __init__(root5, master=None):
			w=Frame.__init__(root5, master, bd=200, height=100, width=200, bg="cyan")
			root5.grid()
			root5.createWidgets()
	
		def createWidgets(root5):
			db1 = MySQLdb.connect(host="localhost",user="root",passwd="shaheer",db="result")
			cursor = db1.cursor()
			...
			for cd in cd_fetch:
				Label(root5, text="REGNO", width=10, anchor=W, bg="green").grid(row=0,column=0)
				for j in range(8):
					Label(root5, text=cd[j], width=10, anchor=W, bg="green").grid(row=0,column=j+1)
				print semstr
			...
			for r in cd2:
					...
					for j in range(9):
						print j
						print r[j]
						Label(root5, text=r[j], width=10, anchor=W, bg="green").grid(row=i+1,column=j)
					i=i+1		
			Button ( root5, text='NEXT', command=barchart, bg="yellow").grid(ipadx=10, column=50,row=20)
		
	root4.destroy()	
	root5.update()
	root5.deiconify()
	app = Application(root5)
...
def sample4():
	class Application(Frame):
		...
		def createWidgets(root4):
			...
			Button ( root4, text='NEXT', command=display, bg="yellow").grid(ipadx=10, column=50,row=20)
	root3.destroy()
	root4.withdraw()	
	root4.update()
	root4.deiconify()	
